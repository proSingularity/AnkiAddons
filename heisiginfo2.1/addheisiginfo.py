...
# which heisig version to use
heisigVersion = '2010' # 2010 or old
...
def lookupKanjiInfo(wordsTxt, key):
    kanjiChars = r'[\u4e00-\u9fbf]'
    words = re.findall(kanjiChars,wordsTxt)
    results = []
    kanjiIndex = readRTK.readRTK()
    for word in words:
        if (word in kanjiIndex):
            # showInfo("found kanji in index: " + word )
